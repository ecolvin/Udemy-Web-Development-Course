...
app.get("/filter", async (req, res) => {
  ...
  res.json(jokes.filter(joke => joke.jokeType === type));
});
...
app.put("/jokes/:id", async (req, res) => {
  ...
  {
    res.status(404).json({
      error: `Joke with id: ${id} not found. No jokes were replaced.`
    });
  }
});
...
app.patch("/jokes/:id", async (req, res) => {
  ...
  {
    res.status(404).json({
      error: `Joke with id: ${id} not found. No jokes were updated.`
    });
  } 
});
